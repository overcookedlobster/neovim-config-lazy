...
function M.open_notes_buffer(persistent)
	...
	if vim.api.nvim_buf_line_count(buf) == 1 and vim.api.nvim_buf_get_lines(buf, 0, -1, false)[1] == "" then
		local title =
			utils.center(persistent and "Persistent Notes (" .. get_project_name() .. ")" or "Temporary Notes")
		local timestamp = utils.right_justify(os.date("%Y-%m-%d %H:%M:%S"))
...
function M.setup(opts)
	...
	-- Keybindings are now managed in lua/config/keymaps.lua for better organization
	-- This avoids conflicts and ensures all keymaps are centralized
